...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        BeginFunction = "function fun(x1,x2,x3,x4,x5,x6,x7,x8)\n {\n var RESULT=0;\n";
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x << 0 << 0 << 0 << 0 << 0 << 0 << 0).toNumber();
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x << y << 0 << 0 << 0 << 0 << 0 << 0).toNumber();
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x << y << z << 0 << 0 << 0 << 0 << 0).toNumber();
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x << y << z << k << 0 << 0 << 0 << 0).toNumber();
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x1 << x2 << x3 << x4 << x5 << 0 << 0 << 0).toNumber();
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x1 << x2 << x3 << x4 << x5 << x6 << 0 << 0).toNumber();
    }
...
*/
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x1 << x2 << x3 << x4 << x5 << x6 << x7 << 0).toNumber();
    }
...
double HarrixClass_EvaluateFunction::getEvaluateFunction (double x1, double x2, double x3, double x4, double x5, double x6, double x7, double x8)
{
    /*
    Получить значение функции в точке (x1,x2,x3,x4,x5,x6,x7,x8).
    Входные параметры:
     x1 - первый входной параметр в восмимерной функции.
     x2 - второй входной параметр в восмимерной функции.
     x3 - третий входной параметр в восмимерной функции.
     x4 - четвертый входной параметр в восмимерной функции.
     x5 - пятый входной параметр в восмимерной функции.
     x6 - шестой входной параметр в восмимерной функции.
     x7 - седьмой входной параметр в восмимерной функции.
     x7 - восьмой входной параметр в восмимерной функции.
    Возвращаемое значение:
     Значение функции в точке.
*/
    Result = 0;
    ...
    if (Type==EvaluateFunctionEightVariables)
    {
        Result = scriptFun.call(QScriptValue(), QScriptValueList() << x1 << x2 << x3 << x4 << x5 << x6 << x7 << x8).toNumber();
    }
