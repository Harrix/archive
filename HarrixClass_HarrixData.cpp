...
{
    /*
    Сколько координат содержится в выбранной линии (и др.) в данных.
    Входные параметры:
     Отсутствуют.
    Возвращаемое значение:
...
int HarrixClass_HarrixData::getNumberElementsInChart(int k)
{
    ...
    без учета их значения. Тут же получение координаты получается согласно семантики конкретного набора данных по его типу.
    Входные параметры:
     k - номер линии  (и др.).
    Возвращаемое значение:
