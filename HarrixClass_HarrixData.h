...
public:
    ...
    QString getType();//узнать тип формата данных
    QString getAxisX();//узнать название оси Ox
    QString getAxisY();//узнать название оси Oy
    QString getAxisZ();//узнать название оси Oz
    QString getTitle();//узнать название графика или набора данных
    QString getNameOfLine(int j);// получить имя данной линии или столбца согласно типу данных

    //если рассматривать данные как просто таблицу
    int getNumberOfRows();//узнать сколько строчек данных содержится в файле
    int getNumberOfCols();//узнать сколько столбцов данных содержится в файле
    int getNumberElementsInCol(int j);//узнать сколько элементов содержится в данном столбце данных
    double getElementOfData(int i, int j);//получить значение элемента из данных согласно типу данных

    //если рассматривать как множество линий, наборов точек, графиков, в которох есть сове число координат
    int getNumberOfCharts();//сколько линий, поверхностей или наборов точек содержится в файле
    int getNumberCoordinateInChart();//Cколько координат содержится в выбранной линии (и др.) в данных
    int getNumberElementsInChart(int k);//Сколько элементов (точек) содержится в выбранной линии (и др.) в данных
    double getPointCoordinateOfData(int k, int i, int j);//получить значение координаты точки из данных
