...
Rectangle {
  ...
  states: [
    State {
      ...
      PropertyChanges {
        target: rectangleColumnMainMenu
        visible: false
      }
      PropertyChanges {
        target: rightIconMouseArea
        colorRipple: colorMenuLight
      }
    },
    State {
      name: "collapsed"
        ...
        if (mainMenuModel.get(i).name === name) {
          ...
          if (submenuArray !== undefined) {
            for (var j = 0; j < submenuArray.count; j++) {
              model.append({
                             caption: submenuArray.get(j).caption,
  ...
  function buildModelVerticalMenu() {
    ...
    for (var i = 0; i < mainMenuModel.count; i++) {
      model.append({
                     caption: mainMenuModel.get(i).caption,
