...
def add_author_book_evernote(filename: Path | str) -> None:
    """
    The function adds the author and the title of the book to the quotes from
    Evernote and forms them as Markdown quotes.
    ...
    > Line 2.
    >
    > _Name Surname - Modified title of book 1_
    ...
    > Line 4.
    >
    > _Name Surname - Modified title of book 2_
    ```
    """
    file = Path(filename)
    if not file.is_file():
...
def main():
    print(
        """Select command
    1 - add_author_book
    2 - add_author_book_evernote
    """
    )
    command = input("Input number of a command: ")
    if command == "1":
        ...
        path = input("Input path: ")
        if Path(path).is_dir():
            modify_all_md_in_folder(path, add_author_book_evernote)
        else:
            print("Invalid path.")
